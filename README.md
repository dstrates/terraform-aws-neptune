...
module "neptune" {
  ...
  engine_version                         = "1.4.7.0"     # Neptune Serverless supported engine is 1.2.0.1+
  ...
  # Security group ingress: use security_groups (preferred) or CIDRs, or both.
  # At least one must be set when create_neptune_security_group = true.
  ingress_security_group_ids = [aws_security_group.app.id]
  # neptune_subnet_cidrs     = ["10.1.0.0/16"]  # CIDR alternative
...
| <a name="input_deletion_protection"></a> [deletion\_protection](#input\_deletion\_protection) | (Optional) A value that indicates whether the DB cluster has deletion protection enabled | `bool` | `true` | no |
...
| <a name="input_engine_version"></a> [engine\_version](#input\_engine\_version) | The database engine version. When enable\_serverless = true, must be 1.2.0.1 or above. | `string` | `"1.2.0.1"` | no |
...
| <a name="input_neptune_cluster_parameters"></a> [neptune\_cluster\_parameters](#input\_neptune\_cluster\_parameters) | A map of Neptune cluster parameter settings | <pre>map(object({<br/>    key          = string<br/>    value        = string<br/>    apply_method = optional(string)<br/>  }))</pre> | <pre>{<br/>  "parameter1": {<br/>    "key": "neptune_enable_audit_log",<br/>    "value": "1"<br/>  }<br/>}</pre> | no |
| <a name="input_neptune_db_parameters"></a> [neptune\_db\_parameters](#input\_neptune\_db\_parameters) | A map of Neptune DB parameter settings | <pre>map(object({<br/>    key          = string<br/>    value        = string<br/>    apply_method = optional(string)<br/>  }))</pre> | <pre>{<br/>  "parameter1": {<br/>    "key": "neptune_query_timeout",<br/>    "value": "25"<br/>  }<br/>}</pre> | no |
...
| <a name="input_neptune_subnet_cidrs"></a> [neptune\_subnet\_cidrs](#input\_neptune\_subnet\_cidrs) | CIDR blocks allowed to reach the Neptune port. Used for the ingress rule on the managed security group. | `list(string)` | `[]` | no |
| <a name="input_neptune_subnet_group_name"></a> [neptune\_subnet\_group\_name](#input\_neptune\_subnet\_group\_name) | (Optional) Name of an existing Neptune subnet group to use when create\_neptune\_subnet\_group = false. | `string` | `null` | no |
